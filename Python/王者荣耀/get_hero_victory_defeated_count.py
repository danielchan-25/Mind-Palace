...
def find_max_age(csv_file):
    ...
    with open(f'赛事\\{csv_file}','r') as file:
        ...
        for row in reader:
            name = row[0]
            victory_count = int(row[4])
            defeated_count = int(row[5])
            if victory_count > max_victory_count:
                max_victory_count = victory_count
                victory_hero_name = name
            if defeated_count > max_defeated_count:
                max_defeated_count = defeated_count
                defeated_hero_name = name

    print(f'在{match_name}中，\n胜场次数最多的英雄是：{victory_hero_name}，胜利场数为：{max_victory_count} 场；')
    print(f'失败次数最多的英雄是：{defeated_hero_name}，失败场数为：{max_defeated_count} 场。\n\n')

for csv_file in csv_file_list:
    find_max_age(csv_file = csv_file)